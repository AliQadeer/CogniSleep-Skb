...
def all_packages(request):
    try:
        pkg = Product_detail.objects.all().order_by("-id")
        notify = Provider.objects.filter(subscription_status='Pending').count()
        n_notify = Provider_Verification.objects.filter(user_position=3).count()
        context = {
            'n_notify': n_notify,
...
#             "package": pkg,
#         }
#         print("opening all coupon page")
#         return render(request, "backend/all_coupon_packages.html", context)
#     except Exception as e:
#         print(e)
#         print("Error in opening all coupon page")
#         return redirect('/backend/dashboard/')
def all_packages_coupon(request):
    try:
        print("all coupon page")
        ...
        for package in pkg:
            ...
            if providers.exists():  # Check if any providers exist before looping
                print("Yes provider exists")
                provider_count = providers.count()
                # provider_data = [
                #     {
                #         'first_name': provider.first_name,
                #         'last_name': provider.last_name,
                #         'email': 'testing',#provider.user.email,  # Ensure user exists
                #         'provider_ref': provider.provider_ref if provider.provider_ref else 'MA',
                #         'total_patients': provider.total_patients,
                #         'subscription_type': provider.subscription_type,
                #     }
                #     for provider in providers
                ...
                for provider in providers:
                    user_data = User.objects.get(email=provider.user)
                    print(user_data)
                    data = {
                        'first_name': provider.first_name,
                        'last_name': provider.last_name,
                ...
                # Handle the case where no providers exist for the package
                print("else in all coupon")
                package.provider_count = 0
